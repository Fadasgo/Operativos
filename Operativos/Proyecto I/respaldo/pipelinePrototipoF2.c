...
char *Palindromos(char *palabra, char *salida);
...
int main() {
  ...
  if (pid == 0) {
    ...
    salida = Palindromos(salida, salida);
    printf("En el hijo: %s\ny len de salida: %d\n", salida, strlen(salida));

    jodedor = strlen(salida);
    //escribimos los palindromos
    write(mypipefd[1],salida , strlen(salida)+jodedor); // proceso hijo es el que escribe
    ...
    read(mypipefd[0],buf ,strlen(salida));
    printf("salida: %s\n", buf);
  }
  return 0;
}
...
char *Palindromos(char *palabra, char *salida){
  ...
  salida = realloc(salida, sizeof(char) * (strlen(final) + 1));
